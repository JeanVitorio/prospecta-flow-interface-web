"""
Importador de leads - Lojas de Veículos → JVS Flow
===================================================
Lê o CSV gerado pelo scraper (lojas_veiculos_sem_site.csv),
...
Como rodar:
  python importar_leads_veiculos.py
...
CSV_PATH             = Path(__file__).parent / "lojas_veiculos_sem_site.csv"
NICHO                = "veiculos"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(Path(__file__).parent / "log_importador_veiculos.txt", encoding="utf-8"),
    ],
)
...
def leads_existentes() -> set[str]:
    """
    Busca todos os leads com source=NICHO já no banco e devolve um set
    de chaves armazenadas no campo notes para deduplicação.
    """
...
# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    log.info("=== Importador de leads - Lojas de Veículos ===")
